...
def main():
    ...
    test_image = REAL_PATH + '/assets/imgTest.jpg'
    overlay_image(test_image, 0, 6, window=[0,0,512,384])
    overlay_image(test_image, 0, 6, window=[513,0,512,384])
    overlay_image(test_image, 0, 6, window=[0,385,512,384])
    overlay_image(test_image, 0, 6, window=[513,385,512,384])
    ...
    for device in devices:
        if device.name == 'AB Shutter3':
            ...
            while True:
                ...
                if event != None and event.type == evdev.ecodes.EV_KEY and event.code == 304 and event.value == 01:
